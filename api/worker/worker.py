...
async def process_job(video_url: str, job_id: str) -> None: 
    ...
    #Steo 1a: Determine if coming from gcs
    if video_url.startswith("gs://"):
        print("Downloading video from GCS...")
        local_video_path : str = download_video(video_url, job_id)
        print("Extracting audio from video...")
        local_audio_path : str = f"/tmp/{job_id}.m4a"
        subprocess.run(["ffmpeg", "-i", local_video_path, "-vn", "-y", local_audio_path], check=True)
        video_title = "Uploaded Video from GCS"
    else:
        local_audio_path, video_title = download_audio(video_url, job_id)
    
    captions = extract_captions(local_audio_path)
    if captions is None:
        print("No captions found, creating with whisper")
        captions = transcribe_with_whisper(local_audio_path, language="en")
    ...
    if video_url.startswith("gs://"): 
        # GCS video already downloaded for audio extraction, so reuse that path
